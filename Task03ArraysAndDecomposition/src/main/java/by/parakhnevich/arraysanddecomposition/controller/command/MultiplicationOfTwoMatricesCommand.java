...
import by.parakhnevich.arraysanddecomposition.service.parser.ParseTwoMatrices;
 ...
 */
public class MultiplicationOfTwoMatricesCommand implements Command{
    @Override
    public String execute(List<String> list) throws FileNotFoundException {
        ...
        return LocaleSingleton.getResourceBundle().getString("FIRST_MATRIX") + '\n'
                + listOfMatrices.get(0).get() + LocaleSingleton.getResourceBundle().getString("SECOND_MATRIX")
                + '\n' + listOfMatrices.get(1).get() + '\n'
                + LocaleSingleton.getResourceBundle().getString("RESULT") + '\n' +
                new MatrixCalculator<Number>().multiply(listOfMatrices.get(0),listOfMatrices.get(1)).get();
    }
}
